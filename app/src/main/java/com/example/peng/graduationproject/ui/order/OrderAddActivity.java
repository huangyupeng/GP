...
public class OrderAddActivity extends BaseActivity implements View.OnClickListener{
    ...
    private final int EVENT_ADD_DEMAND = 11;
        ...
        switch (v.getId()){
            case R.id.txt_right:
                ...
                if (current_address.getText().toString().equals("")){
                    ...
                    showToast("请选择结束时间");
                }else if (edit_square.getText().toString().equals("")) {
                    showToast("请输入面积");
                }else if (edit_reward.getText().toString().equals("")){
                    showToast("请输入参考报酬");
                }else{
                    getProcHandler().sendEmptyMessage(EVENT_ADD_DEMAND);
    ...
    protected boolean procHandlerCallBack(Message msg) {
        ...
        switch (msg.what){
            case EVENT_ADD_DEMAND:
                try {
                    JSONObject params = new JSONObject();

                    DecimalFormat df = new DecimalFormat("#.00");

                    params.put("location", current_address.getText().toString());
                    params.put("lat", latitude);
                    params.put("lng", longitude);
                    params.put("landType", Constants.FARM_TYPE.indexOf(current_type.getText().toString()));
                    params.put("machineType", Constants.MACHINE_TYPE.indexOf(machine_current_type.getText().toString()));
                    params.put("area", df.format(Double.valueOf(edit_square.getText().toString())));
                    params.put("price", df.format(Double.valueOf(edit_reward.getText().toString())));
                    params.put("startTime", current_start_time.getText().toString() + " 00:00:00");
                    params.put("endTime", current_end_time.getText().toString() + " 00:00:00");


                    JSONObject res = NetManager.doPostJson(NetInterface.ADD_DEMAND, params);
